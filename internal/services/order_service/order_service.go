...
func (r *OrderService) UpdateOrderStatus(orderID int, newStatus string) error {
	...
	if newStatus == "returned" && order.Status != "return_requested" {
		return errors.New("order must be in return_requested status to mark as returned")
	}

	return r.orderRepo.UpdateOrderStatus(orderID, newStatus)
...
func (s *OrderService) GetOrderHistory(userCtx *models.UserContext, filterStatus []string) ([]*models.Order, error) {
	return s.orderRepo.GetOrderHistory(userCtx.ID, filterStatus)
}
...
func (s *OrderService) MarkOrderAsReturned(orderID int, userCtx *models.UserContext) error {
	...
	returnRequests, err := s.returnRequestRepo.GetAllReturnRequests([]string{"approved"})
	if err != nil {
	...
	return s.orderRepo.UpdateOrderStatus(orderID, "returned")
}
