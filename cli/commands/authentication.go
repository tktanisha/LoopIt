...
func loginUtil(email, password string, ctx *context.Context) bool {
	...
	_, err = validators.ValidateJWT(token) // optional if already validated during creation
	if err != nil {
	...
	*ctx = context.WithValue(*ctx, constants.UserCtxKey, userCtx)
	...
	fmt.Println(config.Green + "Logged in successfully. JWT: " + token + config.Reset)
